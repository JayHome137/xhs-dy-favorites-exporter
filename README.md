...
小红书和抖音已经合并在同一个 Chrome 扩展中。只需要安装一次；进入不同网站时，扩展会自动加载对应平台的采集逻辑。
...
1. 在 `chrome://extensions` 中禁用或删除旧的小红书插件。
2. 禁用或删除旧的抖音插件。
3. 按上面的步骤加载本仓库的 `extension/` 目录。
4. 回到已经打开的平台页面并刷新一次。

旧插件和合并版同时启用时，可能会出现两个面板或重复采集，因此不建议并存。

### 更新扩展

以后更新仓库代码后，在 `chrome://extensions` 页面点击这个扩展的刷新按钮，再刷新小红书或抖音页面即可。
...
- 抖音接口有 `msToken` / `a_bogus` 等参数，第一版不逆向接口，只读取网页已渲染卡片。
- Obsidian 导入是可选脚本，不绑定扩展。

## 已知限制

- 只导出收藏列表索引，不抓正文、评论、全部图片或视频文件。
- 抖音第一版只读取网页端已渲染的视频卡片；网页没显示的条目不会凭空出现。
